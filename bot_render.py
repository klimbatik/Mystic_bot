...
async def start(message: Message):
    await message.answer(
        ...
        "Я могу рассчитать твой <i>кармический хвост</i>.\n\n"
        "Используй команду:\n"
        "<code>/tail ДД.ММ.ГГГГ</code> — чтобы узнать свой хвост."
    )
...
async def tail_command(message: Message):
    ...
    if len(parts) < 2:
        await message.reply("Укажи дату в формате: <code>/tail ДД.ММ.ГГГГ</code>")
        return
    try:
        day, month, year = map(int, parts[1].split("."))
    except Exception:
        await message.reply("❌ Неверный формат. Используй: <code>ДД.ММ.ГГГГ</code>")
        return
    ...
    await message.answer(
        ...
        f"📌 {description}\n\n"
        f"👉 Подпишись на канал, чтобы получать больше магических знаний: {CHANNEL_LINK}"
    )
